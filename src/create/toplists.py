...
from src.models.providers.mdb import MdbList
...
async def sync_top_lists(config):
    ...
    for top_list in top_lists[:100]:
        ...
        top_list = MdbList(**top_list)  # Validate the list

        log.debug("Creating MdbList list", top_list=top_list)
        ...
        log.debug("Mapped data to MediaList", media_list=media_list)
