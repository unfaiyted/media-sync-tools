# Class that will be able to sync an action to a series of clients.ts


# Example: Omni.createCollection() -> OmniCollection -> which will create a collection
# for the defined clients.ts, one in plex, one in emby, one in jellyfin, etc.
# OmniCollection will have a method to sync the collection to all clients.ts
# OmniCollection will need to know what clients.ts to sync to, and what the collection or list is.

# clients.ts should also include list storing websites like Trakt, IMDB, etc.
# could sync lists across sites.  Could also sync lists across clients.ts.

# Other Methods to add:
# Omni.syncCollection() -> OmniCollection.sync() -> syncs the collection to all clients.ts
# Omni.syncList() -> OmniList.sync() -> syncs the list to all clients.ts
# Omni.savePosters() -> Omni.savePosters() -> saves posters for all clients.ts
# Omni.saveBackdrops() -> Omni.saveBackdrops() -> saves backdrops for all clients.ts
# Omni.getCollection() -> OmniCollection -> gets the collection from all clients.ts
...
class Omni:
    ...
    def _initialize_clients(self):
        ...
        # Filter clients.ts based on supported types
    ...
    def createCollection(self, collection_name):
        ...
        # Sync this data to all clients.ts
